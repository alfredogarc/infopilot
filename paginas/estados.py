...
class Pantalla:
    ...
    def abrir_dialogo(self, e, id=0):
        ...
        self.dlg.open = True
        self.page.update()
        #self.page.dialog.content.controls[0].controls[0].focus()
    ...
    def guardar_cambios(self, e):
        ...
        if not self.nombre_input.value:
            ...
            return
        if id:
            sql = f"UPDATE {self.entidad} SET nombre = '{nuevo_nombre}' WHERE id = {id}"
            print(sql)
            ejecutar_sql(sql)
        else:
            ejecutar_sql(f"INSERT INTO {self.entidad} (nombre) VALUES ('{nuevo_nombre}')")
        self.actualizar_tabla()
        self.cerrar_dialogo(e)

    def eliminar_registro(self, id):
        if validar_registros_tabla("pasos", "idestado", id):
        ...
        self.show_message(f"Estado eliminado")
        self.actualizar_tabla()
